...
# get clean csv with constraints
rooms_data = []
...
def random_position_in_room(bounds, constraints=[]):
    x1,y1,x2,y2 = bounds
    while True:
        x = random.randint(x1, x2-1)
        y = random.randint(y1, y2-1)
        # check constraints for positions
        if "Corner" in constraints:
            corners = [(x1,y1),(x1,y2-1),(x2-1,y1),(x2-1,y2-1)]
            if (x,y) not in corners:
                continue
        if "WallAdjacent" in constraints:
            wall_tiles = [(i,j) for i in range(x1,x2) for j in [y1,y2-1]] + [(i,j) for i in [x1,x2-1] for j in range(y1,y2)]
            if (x,y) not in wall_tiles:
                continue
        if "CenterAxis" in constraints:
            cx = (x1 + x2)//2
            cy = (y1 + y2)//2
            if (x != cx and y != cy):
                continue
...
def make_individual(bsp_rooms):
    ...
    for i, bsp_room in enumerate(bsp_rooms):
        ...
        room_name = room_row["Room"] if room_row.get("Room") else room_row.get("Rooms", f"Room{i}")
        ...
        for me_obj in room_row.get("MutualExclusive",[]):
            if me_obj in placed:
                for other in room_row.get("Exclusive",[]):
                    if other in placed:
                        score -= 0.5
# reward objects/creatures placed
    ...
    for i, bsp_room in enumerate(bsp_rooms):
        ...
        for key in ind.get(room_name,{}):
            if random.random() < mutation_rate:
                ind[room_name][key] = random_position_in_room((x1,y1,x2,y2),
                    constraints=room_row.get("Corner",[])+
                                room_row.get("WallAdjacent",[])+
                                room_row.get("CenterAxis",[]))
    return ind
...
def crossover(parent1, parent2):
    ...
    for room_name in parent1:
        ...
        for key in parent1[room_name]:
            child[room_name][key] = parent1[room_name][key] if random.random() < 0.5 else parent2.get(room_name,{}).get(key, parent1[room_name][key])
    return child

# running the genetic algorithm
def run_ga(bsp_rooms, POPULATION=50, GENERATIONS=50, ELITISM=0.1):
